...
class EnhancedTable extends React.Component {
  ...
  this.state = {
    ...
    async componentDidMount(){
        //$.each(this.props.jsondata,function(index,value){this.state.data.push(value)});
       await axios.get("http://localhost:8080/Summer_Internship_Backend/PopulateTable",{ headers: {'Content-Type': 'application/json'}}).then(response => {  
  ...
  async componentDidMount() {
    //fetch('http://localhost:8080/Summer_Internship_Backend/PopulateTable')
    //.then(response => response.json())
    //.then(data => this.setState({data}));

    this.state.responsejson =
    await axios.get("http://localhost:8080/Summer_Internship_Backend/PopulateTable",
              ...
              {stableSort(data, getSorting(order, orderBy))
                ...
                .map(n => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={event => this.handleClick(event, n.customer_number)}
                      role="checkbox"

...
let EnhancedTableToolbar = props => {
  const { numSelected, classes, OnClickPred } = props;
  ...
  return (
    ...
    >
      ...
      <div className={classes.actions}>
        {numSelected > 0 ? (
          <Button autoid="predict-button" color="primary" onClick={OnClickPred}>
          Predict
      </Button>
        ) : (
...
class EnhancedTable extends React.Component {
  ...
  OnClickPred=(event)=>{
    this.setState({clickedRowsId:this.state.selected});
    console.log('clicked');
     axios
      ...
      .then(response=>{
        ...
        this.setState({predictedAmt:response.data});
      }).catch(err=>{
        console.log(err);
      })
  }
  ...
  render() {
    ...
    return (
      <Paper className={classes.root} >
        <EnhancedTableToolbar numSelected={selected.length} OnClickPred={this.OnClickPred}/>
        <div className={classes.tableWrapper}>
          <Table autoid="invoice-table-collector" className={classes.table} aria-labelledby="tableTitle" stickyHeader="true" >
            <EnhancedTableHead
            ...
            <TableBody >
              {stableSort(data, getSorting(order, orderBy))
                ...
                .map(n => {
                  ...
                  predicted=0;
                  if (this.state.clickedRowsId.includes(n.id) && this.state.predictedAmt.length>0) {
                    predicted=this.state.predictedAmt[count].predictions;
                  }
                  return (
                    <TableRow
                      hover
